...
from sklearn import metrics # Import entire metrics module for dot notation
import shap # Will be imported if SHAP_AVAILABLE
import matplotlib.pyplot as plt
import seaborn as sns # Used in Visualization page
from sklearn.tree import DecisionTreeRegressor # Import for Decision Tree
from xgboost import XGBRegressor # Import for XGBoost
...
# --- Function: train_and_evaluate_model (Consolidated from model_training_revised.py) ---
def train_and_evaluate_model(model_name: str, X: pd.DataFrame, y: pd.Series, params: dict = None, test_size: float = 0.2):
    ...
    if model_name == "Linear Regression":
        ...
        model = DecisionTreeRegressor(random_state=42, **params) # Use **params for flexible hyperparams
    elif model_name == "Random Forest":
        model = RandomForestRegressor(random_state=42, **params) # Use **params for flexible hyperparams
    ...
    # 5. Metric Calculation: Evaluate the model's performance.
    ...
    if MLFLOW_AVAILABLE:
        with mlflow.start_run(run_name=f"{model_name}_{pd.Timestamp.now().strftime('%Y%m%d_%H%M%S')}"):
            ...
            mlflow.sklearn.log_model(model, "model", input_example=X_train[:1]) # Log with input_example
    else:
        st.warning("MLflow not available. Model training will proceed, but metrics and models will not be logged.")
        
    return model, X_train, X_test, y_train, y_test, mse, mae, r2


# --- Utility Function: shap_explain ---
...
def show_classical_ml_page():
    ...
    st.write("Train traditional machine learning models (Linear Regression, Decision Tree, Random Forest, XGBoost). Metrics will be logged to MLflow.")

    # Model selection dropdown
    model_type = st.selectbox(
        "Select a model to train:",
        ["Linear Regression", "Decision Tree", "Random Forest", "XGBoost"],
        key="classical_model_type_select"
    )

    # Hyperparameter inputs based on selected model
    model_params = {}
    if model_type == "Decision Tree":
        model_params["max_depth"] = st.number_input("Max Depth (Decision Tree)", min_value=1, value=5, key="dt_max_depth")
    elif model_type == "Random Forest":
        model_params["n_estimators"] = st.number_input("N Estimators (Random Forest)", min_value=10, value=100, step=10, key="rf_n_estimators")
        model_params["max_depth"] = st.number_input("Max Depth (Random Forest)", min_value=1, value=10, key="rf_max_depth")
    elif model_type == "XGBoost":
        model_params["n_estimators"] = st.number_input("N Estimators (XGBoost)", min_value=10, value=100, step=10, key="xgb_n_estimators")
        model_params["learning_rate"] = st.number_input("Learning Rate (XGBoost)", min_value=0.01, max_value=1.0, value=0.1, step=0.01, key="xgb_learning_rate")
        # Add more XGBoost params as needed

    if st.button(f"🚀 Train {model_type} Model", key="train_selected_model_btn"):
        with st.spinner(f"Training {model_type} model..."):
            try:
                # The X_train and y_train here are already scaled numpy arrays.
                # The train_and_evaluate_model function expects pandas DataFrames/Series for X and y
                # because it performs its own train_test_split internally and needs column names for MLflow input_example.
                # So, we convert them back to DataFrame/Series using `features` list.
                trained_model, _, _, _, _, mse, mae, r2 = \
                    train_and_evaluate_model(model_type, pd.DataFrame(X_train, columns=features), y_train, params=model_params)
                
                st.session_state.trained_models[model_type] = trained_model
                st.session_state.classical_ml_metrics[model_type] = {"MAE": mae, "MSE": mse, "R2": r2}
                st.success(f"✅ {model_type} trained and metrics logged (if MLflow is available).")
            except Exception as e:
    ...
    if st.session_state.trained_models:
        ...
        if model_choice:
            ...
            y_pred = selected_model.predict(X_test) # Predict using the X_test from initial load
            ...
            for feature in features:
                ...
                input_data_dict[feature] = st.number_input(f"Enter {feature}", value=default_value, key=f"input_{feature}_{model_choice}") # Unique key
...
def show_pycaret_automl_page():
    ...
    with col1:
        ...
        available_features = [col for col in df_pycaret.columns if col not in [pycaret_target, "Country"]] # Ensure Country is excluded
...
def show_prediction_page():
    ...
    df_pred_page = st.session_state.df_cleaned.copy() # Use the already cleaned data

    # Data preprocessing specific to this page's training flow
    # Assuming df_pred_page already has 'Status' converted and 'Country' dropped if clean_missing did its job.
    # If not, add df_pred_page["Status"] = le.fit_transform(df_pred_page["Status"]) etc. here.
    # For now, relying on df_cleaned being preprocessed.

    # Feature/Target selection
    list_var = df_pred_page.columns.tolist()
    features_selection = st.sidebar.multiselect("Select Features (X)", list_var, default=[col for col in list_var if col != 'Life expectancy ']) # Note the space
    target_selection = st.sidebar.selectbox("Select Target Variable (Y)", list_var, index=list_var.index('Life expectancy '))
    ...
    # Prepare data
    X = df_pred_page[features_selection]
    y = df_pred_page[target_selection]
    ...
    if MLFLOW_AVAILABLE:
        with mlflow.start_run(run_name=model_name):
            ...
            mlflow.log_metric("mae", mae)
            mlflow.log_metric("r2", r2)
            st.success(f"Model '{model_name}' trained and metrics logged to MLflow.")
    else:
        st.warning("MLflow not available. Training model without tracking.")
        model.fit(X_train, y_train)
        predictions = model.predict(X_test)
        mse = metrics.mean_squared_error(y_test, predictions)
        mae = metrics.mean_absolute_error(y_test, predictions)
        r2 = metrics.r2_score(y_test, predictions)
        st.info(f"Model '{model_name}' trained. Metrics not logged due to MLflow unavailability.")
...
def show_explainability_page():
    ...
    fig_waterfall, ax_waterfall = plt.subplots() # Create a new figure
    shap.plots.waterfall(shap_values[0], show=False)
    st.pyplot(plt.gcf()) # Use plt.gcf() to get the current figure
    plt.close(fig_waterfall) # Close the figure to prevent memory issues


    # SHAP Scatter Plot for 'Latitude'
    st.markdown("### SHAP Scatter Plot for 'Latitude'")
    fig_scatter, ax_scatter = plt.subplots() # Create a new figure
    shap.plots.scatter(shap_values[:, "Latitude"], color=shap_values, show=False)
    st.pyplot(plt.gcf()) # Use plt.gcf() to get the current figure
    plt.close(fig_scatter) # Close the figure to prevent memory issues
...
if page_selection == "Home":
    ...
    with col2:
        ...
        - Make interactive predictions on the **Prediction 🤖** page.
...
elif page_selection == "Prediction 🤖": # New page selection
